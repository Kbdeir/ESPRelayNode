...
bool onP003PS()
{
  ...
  SERIAL_DBG.print("Processing data from: ");
  SERIAL_DBG.println(_lastRequestedCommand);
...
bool onMOD()
{
  SERIAL_DBG.print(F("QMOD '"));
  SERIAL_DBG.print(_commandBuffer);
  SERIAL_DBG.print(F("'"));
...
bool onOther()
{
  SERIAL_DBG.print("Processing OTHER data from: ");
  SERIAL_DBG.println(_lastRequestedCommand);
...
void onInverterCommand()
{
  if ((_commandBuffer.length() > 3) )
  {
    ...
    if (crcCheck) {
      SERIAL_DBG.print(F(" Calc: "));
      SERIAL_DBG.print(calculatedCrc, HEX);
      SERIAL_DBG.print(F(" Rx: "));
      SERIAL_DBG.println(recievedCrc, HEX);
    }
    SERIAL_DBG.print(F("Command sent: "));
    SERIAL_DBG.print(_lastRequestedCommand);
    SERIAL_DBG.print(F(" Recieved data: "));
    SERIAL_DBG.println(_commandBuffer);
...
void serviceInverter()
{
  ...
  if ((_lastRequestedCommand == "") && (_lastReceivedAt.compare(INVERTER_COMMAND_DELAY_MS) > 0) && (!_allMessagesUpdated))
  {
    ...
    SERIAL_DBG.print(F("Sent Command: "));
    if (inverterType) { SERIAL_INVERTER.print("^"); SERIAL_DBG.print("^");}  //If MPI then prechar is needed
    SERIAL_INVERTER.print(_nextCommandNeeded);
    SERIAL_DBG.println(_nextCommandNeeded);
    if (crcCheck) SERIAL_INVERTER.print((char)((crc >> 8) & 0xFF)); //ONLY CRC fo PCM/PIP
    if (crcCheck) SERIAL_INVERTER.print((char)((crc >> 0) & 0xFF)); //ONLY CRC fo PCM/PIP
    if (crcCheck) SERIAL_DBG.println(F("Doing the CRC"));
    SERIAL_INVERTER.print("\r");
   
   if (_setCommand.length()) { _nextCommandNeeded = ""; _setCommand = ""; }  // If it was RAW command reset it.
  }
 
  while (SERIAL_INVERTER.available() > 0)
  {
    c = SERIAL_INVERTER.read();
