...
void WireGuard::end() {
	...
	wireguardif_remove_peer(wg_netif, wg_peer_index);
...
bool WireGuard::is_peer_up() const {
	if (!this->_is_initialized || wg_netif == nullptr || wg_peer_index == WIREGUARDIF_INVALID_INDEX) return false;
	return wireguardif_peer_is_up(wg_netif, wg_peer_index, NULL, NULL) == ERR_OK;
}

bool WireGuard::get_peer_endpoint(IPAddress& address, uint16_t& port) const {
	if (!this->_is_initialized || wg_netif == nullptr || wg_peer_index == WIREGUARDIF_INVALID_INDEX) return false;

	ip_addr_t current_ip = IPADDR4_INIT_BYTES(0, 0, 0, 0);
	u16_t current_port = 0;
	if (wireguardif_peer_is_up(wg_netif, wg_peer_index, &current_ip, &current_port) != ERR_OK) return false;

	const uint32_t raw = current_ip.u_addr.ip4.addr;
	address = IPAddress(
		(raw >> 0) & 0xff,
		(raw >> 8) & 0xff,
		(raw >> 16) & 0xff,
...
uint32_t WireGuard::session_age_ms() const {
	if (!this->_is_initialized || wg_netif == nullptr || wg_peer_index == WIREGUARDIF_INVALID_INDEX) return UINT32_MAX;
	struct wireguard_device *device = (struct wireguard_device *)wg_netif->state;
	if (device == nullptr) return UINT32_MAX;
	struct wireguard_peer *peer = peer_lookup_by_peer_index(device, wg_peer_index);
	if (peer == nullptr) return UINT32_MAX;
	uint32_t now = wireguard_sys_now();
	if (peer->curr_keypair.valid) return now - peer->curr_keypair.keypair_millis;
	if (peer->prev_keypair.valid) return now - peer->prev_keypair.keypair_millis;
	return UINT32_MAX;
}
