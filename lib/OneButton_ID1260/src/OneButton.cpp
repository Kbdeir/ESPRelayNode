...
void OneButton::attachClick(callbackFunction_obj newFunction)
{
  _clickFunc_obj = newFunction;
} // attachClick
 ...
 * machine (FSM).
 */
void OneButton::tick(void* sender)
{
  if (_pin >= 0) {
    tick((digitalRead(_pin) == _buttonPressed), sender);
  }
}
 ...
 */
void OneButton::tick(bool activeLevel, void* sender)
{
  ...
  if (_state == 0) { // waiting for menu pin being pressed.
    ...
    if (_doubleClickFunc == NULL ||
      ...
      if (_clickFunc_obj) _clickFunc_obj(sender);
