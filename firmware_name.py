...
if profile:
    def copy_firmware(source, target, env):
        ...
        dst = os.path.join(build_dir, f"firmware_{profile}.bin")
        if os.path.exists(src):
            shutil.copy2(src, dst)
            print(f"[firmware_name] Created: firmware_{profile}.bin")

    env.AddPostAction("$BUILD_DIR/firmware.bin", copy_firmware)
    print(f"[firmware_name] Post-build: will produce firmware_{profile}.bin")
else:
